...
bool eval_solution_IGA(const MySolution& p, MyMiddleCost& c, const EA::GenerationType<MySolution, MyMiddleCost>&) {
    c.R = p.R;
    c.G = p.G;
    c.B = p.B;
...
void SO_report_generation(
    ...
    output_file << generation_number << "\t" << best_genes.to_string() << "\t" << 100.0 - last_generation.average_cost
